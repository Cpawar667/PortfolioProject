...
public abstract class WorkoutPlannerKernelTest {
    ...
    /**
     * Invokes the appropriate {@code WorkoutPlannerKernel} constructor for the
    ...
    /**
     * Creates a {@code WorkoutPlannerKernel} object with the given muscle group
     * states.
     *
     * @param groups
    ...
    public final void testRemoveSingleEntry() {
        ...
        WorkoutPlannerKernel pRef = this.createPlanner(
                ...
                new WorkoutPlannerKernel.SorenessLevel[] { NONE });
        WorkoutPlannerKernel pTest = this.createPlanner(
                new WorkoutPlannerKernel.MuscleGroup[] { CORE },
                new WorkoutPlannerKernel.SorenessLevel[] { NONE });
    ...
    public final void testRemoveMultiEntry() {
        ...
        WorkoutPlannerKernel pExpected = this.createPlanner(
                ...
                new WorkoutPlannerKernel.SorenessLevel[] { HIGH });

        // Call method (remove SHOULDERS)
        WorkoutPlannerKernel.SorenessLevel removed = pTest.remove(SHOULDERS);
    ...
    public final void testRemoveAnyMultiEntry() {
        ...
        boolean wasRemoved = (removed.key() == CHEST && removed.value() == LOW)
                || (removed.key() == BACK && removed.value() == MEDIUM);
    ...
    public final void testSizeMultiEntry() {
        WorkoutPlannerKernel.MuscleGroup[] groups = { CHEST, BACK, LEGS, CORE };
        WorkoutPlannerKernel.SorenessLevel[] levels = { LOW, MEDIUM, HIGH,
                NONE };
        WorkoutPlannerKernel pTest = this.createPlanner(groups, levels);
    ...
    public final void testClearNonEmpty() {
        // Setup initial state (non-empty)
        WorkoutPlannerKernel pTest = this.createPlanner(
                new WorkoutPlannerKernel.MuscleGroup[] { CHEST, BACK },
                new WorkoutPlannerKernel.SorenessLevel[] { LOW, MEDIUM });
        WorkoutPlannerKernel pExpected = this.constructorRef();
    ...
    /**
     * Test transferFrom copies content and clears source.
     */
    @Test
    public final void testTransferFrom() {
        // Setup initial state for target: {CORE -> NONE}
        WorkoutPlannerKernel pTest = this.createPlanner(
                new WorkoutPlannerKernel.MuscleGroup[] { CORE },
                new WorkoutPlannerKernel.SorenessLevel[] { NONE });

        // Setup initial state for source: {LEGS -> HIGH, ARMS -> LOW}
        WorkoutPlannerKernel.MuscleGroup[] sourceGroups = { LEGS, ARMS };
        WorkoutPlannerKernel.SorenessLevel[] sourceLevels = { HIGH, LOW };
        WorkoutPlannerKernel pSource = this.createPlanner(sourceGroups,
                sourceLevels);

        // Setup expected state for target: {LEGS -> HIGH, ARMS -> LOW}
        WorkoutPlannerKernel pExpected = this.createPlanner(sourceGroups,
                sourceLevels);

        // Setup expected state for source (empty)
        WorkoutPlannerKernel pExpectedSource = this.constructorRef();

        // Call method (transfers source to test, clearing source)
        pTest.transferFrom(pSource);

        // Evaluation
        assertEquals(pExpected, pTest);
        assertEquals(pExpectedSource, pSource);
    }
}
